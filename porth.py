...
def type_check_program(program: Program):
    ...
    while len(contexts) > 0:
        ...
        assert len(OpType) == 16, f"Exhaustive ops handling in type_check_program(): (expected: {len(OpType)})"
        if op.typ == OpType.PUSH_INT:
...
def generate_nasm_linux_x86_64(program: Program, out_file_path: str):
    ...
    with open(out_file_path, "w") as out:
        ...
        for ip in range(len(program.ops)):
            op = program.ops[ip]
            assert len(OpType) == 16, "Exhaustive ops handling in generate_nasm_linux_x86_64: %d" % len(OpType)
            out.write("addr_%d:\n" % ip)
...
def eval_const_value(rtokens: List[Token], macros: Dict[str, Macro], consts: Dict[str, Const], iota: List[int]):
    stack: List[int] = []
    while len(rtokens) > 0:
        token = rtokens.pop()
        ...
        if token.typ == TokenType.WORD:
            ...
            if token.value == Keyword.IF:
                ...
                if program.ops[if_ip].typ == OpType.IF:
                    ...
                    else_ip = -69 if len(stack) == 0 else stack.pop()
                    assert else_ip != -69 and program.ops[else_ip].typ == OpType.ELSE, "At this point we should've already checked that `if*` comes after `else`. Otherwise this is a compiler bug."
                    program.ops[if_ip].operand = ip + 1
